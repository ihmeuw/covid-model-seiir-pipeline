...
def create_model_data(cumulative_deaths: pd.Series,
    ...
    loc_dates = ifr_data.reset_index()[['location_id', 'date']].drop_duplicates().values.tolist()
    ...
    model_data = ifr_data.to_frame().join(dates_data.set_index(['location_id', 'date']), how='left')
    ...
    if model_data['ratio_data_scalar'].isnull().any():
        raise ValueError('Missing IFR data scalar.')
