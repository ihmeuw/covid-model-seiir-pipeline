...
class CounterfactualDataInterface:
    ...
    def get_infections_metadata(self):
        return self._get_forecast_data_inteface().get_infections_metadata()

    def get_model_inputs_metadata(self):
        return self._get_forecast_data_inteface().get_model_inputs_metadata()
    ...
    def load_total_deaths(self) -> pd.Series:
        return self._get_forecast_data_inteface().load_total_deaths()
    ...
    def load_covariates(self, covariates: Dict[str, str]) -> pd.DataFrame:
        return self._get_forecast_data_inteface().load_covariates(
            covariates,
        )

    def load_vaccinations(self, vaccine_scenario: str) -> pd.DataFrame:
        return self._get_forecast_data_inteface().load_vaccinations(
            vaccine_scenario,
        )
    ...
    def load_mobility_info(self, info_type: str) -> pd.DataFrame:
        return self._get_forecast_data_inteface().load_mobility_info(
            info_type,
        )

    def load_mandate_data(self, mobility_scenario: str) -> Tuple[pd.DataFrame, pd.DataFrame]:
        return self._get_forecast_data_inteface().load_mandate_data(
            mobility_scenario,
        )

    def load_variant_prevalence(self, variant_scenario: str) -> pd.DataFrame:
        return self._get_forecast_data_inteface().load_variant_prevalence(
            variant_scenario,
        )
    ...
    def load_coefficients(self, draw_id: int) -> pd.DataFrame:
        return self._get_forecast_data_inteface().load_coefficients(draw_id=draw_id)

    def load_compartments(self, draw_id: int) -> pd.DataFrame:
        return self._get_forecast_data_inteface().load_compartments(draw_id=draw_id)

    def load_ode_parameters(self, draw_id: int) -> pd.DataFrame:
        return self._get_forecast_data_inteface().load_ode_parameters(draw_id=draw_id)

    def load_past_infections(self, draw_id: int) -> pd.Series:
        return self._get_forecast_data_inteface().load_past_infections(draw_id=draw_id)
    ...
    def load_past_deaths(self, draw_id: int) -> pd.Series:
        return self._get_forecast_data_inteface().load_past_deaths(draw_id=draw_id)

    def get_hospital_parameters(self) -> HospitalParameters:
        return self._get_forecast_data_inteface().get_hospital_parameters()

    def load_hospital_usage(self) -> HospitalMetrics:
        return self._get_forecast_data_inteface().load_hospital_usage()

    def load_hospital_correction_factors(self) -> HospitalCorrectionFactors:
        return self._get_forecast_data_inteface().load_hospital_correction_factors()
    ...
    def load_ifr(self, draw_id: int) -> pd.DataFrame:
        return self._get_forecast_data_inteface().load_ifr(draw_id=draw_id)

    def load_ihr(self, draw_id: int) -> pd.DataFrame:
        return self._get_forecast_data_inteface().load_ihr(draw_id=draw_id)

    def load_idr(self, draw_id: int) -> pd.DataFrame:
        return self._get_forecast_data_inteface().load_idr(draw_id=draw_id)

    def load_ratio_data(self, draw_id: int) -> RatioData:
        return self._get_forecast_data_inteface().load_ratio_data(draw_id=draw_id)
