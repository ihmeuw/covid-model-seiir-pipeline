...
def compute_output_metrics(indices: Indices,
    ...
    components = (components
                  ...
                  .fillna(method='bfill'))
    metrics_map = {
        'normal': normal_system_metrics,
        'vaccine': vaccine_system_metrics,
    }
    system_metrics_maker = metrics_map.get(system, variant_system_metrics)
...
def normal_system_metrics(indices: Indices,
    ...
    return SystemMetrics(
        ...
        vaccinations_effective=pd.Series(np.nan, index=indices.full),
        vaccinations_ineffective=pd.Series(np.nan, index=indices.full),

        total_susceptible_wild=components['S'],
...
def vaccine_system_metrics(indices: Indices,
    ...
    total_susceptible = components.loc[:, [c for c in components.columns if 'S' in c]]
    total_immune = components.loc[:, [c for c in components.columns if 'R' in c]]
    ...
    return SystemMetrics(
        ...
        total_susceptible_wild=total_susceptible,
        total_susceptible_variant=pd.Series(np.nan, index=indices.full),
...
def variant_system_metrics(indices: Indices,
    ...
    natural_immunity_breakthrough = components_diff[[c for c in cols if 'NewE_nbt' in c]].sum(axis=1)
    vaccine_breakthrough = components_diff[[c for c in cols if 'NewE_vbt' in c]].sum(axis=1)
    new_s_variant = components_diff[[c for c in cols if 'NewS_v' in c]].sum(axis=1)
    new_r_wild = components_diff[[c for c in cols if 'NewR_w' in c]].sum(axis=1)
    ...
    vaccines_u = components_diff[[c for c in cols if 'V_u' in c]].sum(axis=1)
    vaccines_p = components_diff[[c for c in cols if 'V_p' in c and 'pa' not in c]].sum(axis=1)
    vaccines_pa = components_diff[[c for c in cols if 'V_pa' in c]].sum(axis=1)
    vaccines_m = components_diff[[c for c in cols if 'V_m' in c and 'ma' not in c]].sum(axis=1)
    vaccines_ma = components_diff[[c for c in cols if 'V_ma' in c]].sum(axis=1)
...
def compute_corrected_hospital_usage(admissions: pd.Series,
    ...
    hfr = postprocessing_parameters.ifr / postprocessing_parameters.ihr
