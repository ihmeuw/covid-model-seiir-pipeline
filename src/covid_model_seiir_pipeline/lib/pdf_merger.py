...
def merge_pdfs(plot_cache: Path, output_path: Path, patterns: List[str], hierarchy: pd.DataFrame):
    ...
    with PdfFileMerger() as merger:
        ...
        for location_id in sorted_locations:
            p1_path = plot_cache / f"{patterns[0].format(location_id=location_id)}.pdf"
            ...
            merger.merge(current_page, str(p1_path))
            ...
            for i, page_pattern in enumerate(patterns[1:]):
                page_path = plot_cache / f"{page_pattern.format(location_id=location_id)}.pdf"
                merger.merge(current_page + i + 1, str(page_path))

            merged.append(location_id)
            current_page += len(patterns)
