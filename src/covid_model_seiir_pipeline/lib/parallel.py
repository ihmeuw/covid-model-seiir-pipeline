...
def load_and_concat_draws(loader: Callable[[int, Optional[List[str]]], pd.DataFrame],
                          index: pd.Index = None,
                          column: str = None,
                          n_draws: int = 1,
                          num_cores: int = 1,
    ...
    _runner = functools.partial(
        ...
        loader=loader,
        index=index,
        columns=[column] if column is not None else column,
    )
    result = run_parallel(
        runner=_runner,
    ...
    result.columns = [f'draw_{draw}' for draw in range(n_draws)]
    return result
...
def draw_runner(draw_id: int,
                ...
                columns: Optional[List[str]]) -> pd.Series:
    """Loads a draw dataset or a column subset of a draw dataset, and reindexes the dataset if appropriate."""
    if columns and 'round' in index.names:
        columns += ['round']
