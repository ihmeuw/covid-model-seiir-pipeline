...
ENV_NAME=seir_conda
CONDA_PREFIX=/ihme/homes/azimmer/miniconda3/
...
install_env:
	( \
		...
		cd SEIRPipeline && git checkout dev_nathenry && python setup.py install && cd .. && \
		git checkout dev_aoz && python setup.py install; \
    )
